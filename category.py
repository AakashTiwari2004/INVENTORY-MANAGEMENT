...
class categoryClass:
 def __init__(self,root):
     ...
     #======title======== 
     lbl_title=Label(self.root,text="Manage Product Category",font=("goudy old style",30),bg="blue",fg="white",bd=3,relief=RIDGE).pack(side=TOP,fill=X,padx=10,pady=20)
     lbl_name=Label(self.root,text="Enter Category Name",font=("goudy old style",30),bg="orange",bd=3,relief=RIDGE).place(x=50,y=100)
   ...
   #====category Details===
    ...
    #=====images=======
     self.im1=Image.open("C:\\Users\\great\\OneDrive\\Desktop\\Inventory Mangement\\prestashop-product-category.png")
     self.im1=self.im1.resize((450,220),Image.Resampling.LANCZOS)
     self.im1=ImageTk.PhotoImage(self.im1)

     self.lbl_im1=Label(self.root,image=self.im1,bd=2,relief=RIDGE)
     self.lbl_im1.place(x=150,y=250)
     self.show()
 ...
 def add(self): 
    ...
    try:
        if self.var_name.get()=="":
            ...
            row=cur.fetchone()
            if row!=None:
                messagebox.showerror("Error","Category already available, try different",parent=self.root)
            else:
